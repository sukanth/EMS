...
public class User_dao {
	...
	public int addexpense(User_bean expense) {
		...
		Object params[] = new Object[] { expense.getUsername(),
				expense.getName(), expense.getDate(),expense.getApplicableto(),expense.getAmount(),expense.getDatta(),expense.getKiran(),expense.getGunda(),expense.getVamsi(),expense.getChowdary(),expense.getUday(),expense.getVihar(),"ACTIVE",expense.getId()};
	...
	public List<IndividualExpense> getCalculations(String username) {
		...
		List<IndividualExpense> cal=jdbc.query("select sum(datta) datta,sum(gunda) gunda,sum(kiran) kiran,sum(uday) uday,sum(chowdary) chowdary,sum(vamsi) vamsi,sum(vihar) vihar,username from expense where status_cd='ACTIVE'  AND username=? GROUP BY username",new Object[]{username}, new CalculationRowMapper());
		return cal;
	}
